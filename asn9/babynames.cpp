...
/**
    Reads name informations, prints the name is total >= 0, and adjusts the total.
...
void process_name(ifstream& in_file, double& total) {
    ...
    if (in_file.fail()) {
        return; // Check for failure after each input
    }
    if (total > 0) {
        cout << name << " ";
    }
...
int main()
{
    ...
    if (in_file.fail()) {
        return 0; // Check for failures after opening
    }
